/*
  con4a4d MIDI Controller (Hybrid MIDI Version)  
...
// I2C
#define I2C_SLAVE_ADDR  HMMAddr_3      // i2c slave address (0x33)
...
const uint8_t numbers[8] = {47,46,45,43,84,85,86,87};
const uint8_t types[8] = {0,0,0,0,1,1,1,1}; // 0:button, 1: volume
...
void loop() {
  ...
  if(types[channel] == 0){
    ...
    if((value == HIGH)&&(value != values[channel])){
      ledOn();
      HybridMidi.sendNoteOn(0,numbers[channel],100);
    }else if((value == LOW)&&(value != values[channel])){
      ledOff();
      HybridMidi.sendNoteOff(0,numbers[channel]);
    }
